...
class Index(View):
    ...
    def get(self, request):
        ...
        camps = request.user.groups.filter(name='campains')
...
class mainCats(View):
    ...
    def get(self, request):
        template = 'admins/campain_categoria.html'
        context = {}

        categorias = Campaincat.objects.filter(parentcat__isnull=True)
        pages = Pageinfo.objects.all().first()
        context['cats'] = categorias
        context['pages']=pages
        response = render(request,template, context)
             ...
             'subs':[
                {
                 ...
                 for s in mc.campaincat_set.all()
             ],
             'slug':mc.catslug
            }
...
catList = catList.as_view()



class rmCampaincat(View):
    ...
    def post(self, request):
        ...
        pk = data.get('pk',None)
        if pk:
            c = Campaincat.objects.get(pk=pk)
            c.delete()

        context['deleted'] = u'Se eliminó con éxito'

        response = JsonResponse(context,safe=False)
        return response

rmCampaincat = rmCampaincat.as_view()




class addCampaincat(View):
    ...
    def post(self,request):
        ...
        namecat = data.get('catname',None)
        catslug = slugify(namecat)
        parentcat = data.get('parentcat',None)

        context = {}

        if namecat:
            ca = Campaincat.objects.filter(catslug=catslug).count()
            if ca > 0:
                context['err'] = u'Ya existe una categoría con esenombre'
                response = JsonResponse(context)
                return response
            else:
                newcat = Campaincat()
                newcat.namecat = namecat
                newcat.catslug = catslug
                if parentcat:
                    newcat.parentcat_id = parentcat

                newcat.save()

                context['saved'] = newcat.pk

                response = JsonResponse(context)
                return response
        else:
            response = JsonResponse({'nocat': 'cat'})
            return response
    


addCampaincat = addCampaincat.as_view()
...
class saveExterno(View):

    def post(self,request):

        data = request.POST.copy()
        filefiles = request.FILES.copy()
        archivo = filefiles.get('file',None)
        if data.get('modulopk')==0 or data.get('modulopk')=='0' or not data.get('modulopk'):
            externo = Externo()
            externo.rowpk_id = data.get('rowpk')
            externo.title=''
            externo.typofile = archivo.content_type
            externo.originalname = archivo.name
            externo.archivomain = archivo           
            externo.save()
            context = {
                'saved':'ok',
                'callback':'newexterno',
                'datos':{
                'pk':externo.pk,
                'filepath':externo.archivomain.url}
            }
            response = JsonResponse(context)
            return response


saveExterno = saveExterno.as_view()
...
class getExterno(View):

    def get(self,request):
        context = {}
        data = request.GET.copy()
        exs = Externo.objects.filter(rowpk_id=data.get('rowpk'))
        exts = [
                {
                'pk':x.pk,
                'originalname':x.originalname,
                'path':x.archivomain.url,
                'typo':x.typofile
                }
                for x in exs
        ...
        response = JsonResponse(exts,safe=False)
        return response


getExterno = getExterno.as_view()



class rmModuloEx(View):
    def get(self,request):
        data = request.GET.copy()
        r =  Rowsection.objects.get(pk=data.get('pk'))
        r.delete()
        context = {'errors':u'Eliminado con éxito'}
        response = JsonResponse(context,safe=False)
        return response
rmModuloEx = rmModuloEx.as_view()
...
class rmEx(View):
    def get(self,request):
        data = request.GET.copy()
        r =  Externo.objects.get(pk=data.get('pk'))
        os.remove(r.archivomain.path)
        r.delete()
...
class CampPortada(View):
    def post(self,request):
        data = request.POST.copy()
        context = {}

        page = Pageinfo.objects.all()
        for x in page:
            os.remove(x.imagen.path)
        page.delete()
        page = Pageinfo()
        page.imagen = request.FILES['file']
        page.save()

        context['img'] = page.imagen.url



        response = JsonResponse(context,safe=False)
        return response
CampPortada = CampPortada.as_view()